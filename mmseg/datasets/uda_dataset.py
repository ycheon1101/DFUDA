...
# Modifications: Support synchronized crop, valid_pseudo_mask, and SAM masks
# ---------------------------------------------------------------
...
class UDADataset(object):
    ...
    def __init__(self, source, target, cfg):
        ...
        self.skip_missing_sam_mask = cfg.get('skip_missing_sam_mask', False)
    ...
    def _attach_sam_mask(self, out, target_sample):
        if 'gt_semantic_seg' in target_sample:
            out['sam_mask'] = target_sample['gt_semantic_seg']
        elif self.skip_missing_sam_mask:
            h, w = target_sample['img'].data.shape[1:]
            out['sam_mask'] = DC(
                torch.zeros((1, h, w), dtype=torch.int64), stack=True)
        return out
    ...
    def get_rare_class_sample(self):
        ...
        out = self._attach_sam_mask(out, s2)
