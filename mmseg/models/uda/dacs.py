...
# - Add DINOv3 prototype contrastive loss and SAM pseudo-label refinement
# ---------------------------------------------------------------
...
class PrototypeContrastiveLoss(nn.Module):
    ...
    def forward(self, proto, feat, labels, tau=0.07):
        mask = labels != 255
        labels = labels[mask].long()
        feat = feat[mask]
        feat = F.normalize(feat, p=2, dim=1)
        proto = F.normalize(proto, p=2, dim=0)
        logits = feat.mm(proto) / max(float(tau), 1e-6)
        return F.cross_entropy(logits, labels)


def load_dino_prototypes(proto_path, device):
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        self.use_dino_proto = cfg.get('use_dino_proto', False)
        self.use_sam = cfg.get('use_sam', False)
        self.sam_start_iter = cfg.get('sam_start_iter', 80000)
        self.proto_path = cfg.get(
            'proto_path', 'prototypes/prototypes_19classes_orig.pt')
        self.proto_lambda = cfg.get('proto_lambda', 0.1)
        self.sam_entropy_threshold = cfg.get('sam_entropy_threshold', 0.99)
        self.proto_proj_in_channels = cfg.get('proto_proj_in_channels', 2048)
        ...
        self.class_probs = {}
        self.cont_loss = None
        self.proj_head_student = None
        self.proto = None
        if self.use_dino_proto:
            self.cont_loss = PrototypeContrastiveLoss()
            self.proj_head_student = nn.Sequential(
                nn.Conv2d(self.proto_proj_in_channels, 1024, 1))
    ...
    def _ensure_proto_loaded(self, device):
        if self.proto is None:
            self.proto = load_dino_prototypes(self.proto_path, device)

    def _get_proto_feat(self, feat):
        """Return the last-scale feature map for DINO prototype loss."""
        while isinstance(feat, (list, tuple)):
            feat = feat[-1]
        return feat

    def calc_proto_loss(self, feat, labels):
        feat = self._get_proto_feat(feat)
        self._ensure_proto_loaded(feat.device)
        b, _, hs, ws = feat.size()
        feat = self.proj_head_student(feat)
        src_mask = F.interpolate(
            labels.float(), size=(hs, ws),
            mode='nearest').squeeze(1).long()
        src_mask = src_mask.contiguous().view(b * hs * ws)
        src_feat = feat.permute(0, 2, 3, 1).contiguous().view(
            b * hs * ws, feat.shape[1])
        loss = self.cont_loss(self.proto, src_feat, src_mask)
        feat_loss, feat_log = self._parse_losses({'loss_cont': loss})
        feat_log.pop('loss', None)
        return feat_loss, feat_log

    def refine_pseudo_labels_with_sam(self, ema_softmax, sam_mask):
        pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1)
        sam_mask_np = sam_mask.detach().cpu().numpy().squeeze()
        if sam_mask_np.ndim == 2:
            sam_mask_np = sam_mask_np[None, ...]
        ema_softmax_np = ema_softmax.detach().cpu().numpy()
        pl_updated = pseudo_label.detach().clone()
        pl_prob_updated = pseudo_prob.detach().clone()
        batch_size = sam_mask_np.shape[0]
        for batch_idx in range(batch_size):
            for mask_id in np.unique(sam_mask_np[batch_idx]):
                if mask_id == 0:
                    continue
                mask_pixels = sam_mask_np[batch_idx] == mask_id
                if mask_pixels.sum() == 0:
                    continue
                mask_indices = np.where(mask_pixels)
                mask_pred = ema_softmax_np[
                    batch_idx, :, mask_indices[0], mask_indices[1]]
                mask_pred_prob = mask_pred.max(axis=1)
                sorted_logits = np.sort(mask_pred, axis=1)
                confidence = sorted_logits[:, -1] - sorted_logits[:, -2]
                confident_pixels = (
                    (confidence >= self.sam_entropy_threshold) &
                    (mask_pred_prob >= self.pseudo_threshold))
                if confident_pixels.sum() == 0:
                    continue
                predicted_classes = np.argmax(mask_pred, axis=1)[confident_pixels]
                if np.all(predicted_classes == predicted_classes[0]):
                    final_class = int(predicted_classes[0])
                    rows, cols = np.where(mask_pixels)
                    pl_updated[batch_idx, rows, cols] = final_class
                    mean_prob = ema_softmax[
                        batch_idx, final_class, rows, cols].mean()
                    pl_prob_updated[batch_idx, rows, cols] = mean_prob
        ps_large_p = pl_prob_updated.ge(self.pseudo_threshold).long() == 1
        ps_size = np.size(np.array(pl_updated.cpu()))
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size
        pseudo_weight = pseudo_weight * torch.ones(
            pseudo_prob.shape, device=ema_softmax.device)
        return pl_updated, pseudo_weight
    ...
    def forward_train(self,
        ...
        retain = self.enable_fdist or self.use_dino_proto
        clean_loss.backward(retain_graph=retain)
        ...
        if self.enable_fdist:
            ...
            feat_loss.backward(retain_graph=self.use_dino_proto)
            if self.print_grad_magnitude:
                params = self.get_model().backbone.parameters()
                fd_grads = [
        ...
        if self.use_dino_proto:
            cont_loss, cont_log = self.calc_proto_loss(
                src_feat, gt_semantic_seg)
            cont_loss = self.proto_lambda * cont_loss
            cont_loss.backward()
            log_vars.update(add_prefix(cont_log, 'src_cont'))
        ...
        if not self.source_only:
            ...
            if (self.use_sam and sam_mask is not None
                    and self.local_iter >= self.sam_start_iter):
                ema_softmax = torch.softmax(ema_logits.detach(), dim=1)
                pseudo_label, pseudo_weight = self.refine_pseudo_labels_with_sam(
                    ema_softmax, sam_mask)
            else:
